...
///
/// Used by [AdvDropdownButton.selectedItemBuilder].
typedef AdvDropdownButtonBuilder = List<Widget> Function(BuildContext context);
...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // );
    /// change start
    final buildToBottom = menuLimits?.buildToBottom ?? true;
...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    final buildToBottom = menuLimits.buildToBottom;
...
class _MenuLimits {
  const _MenuLimits(this.top, this.bottom, this.height, this.scrollOffset, this.buildToBottom);
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>?> {
  ...
  Widget buildPage(BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        menuLimits = getMenuLimits(buttonRect, constraints.maxHeight, selectedIndex);
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight, int index) {
    const maxMenuHeight = 5 * _kMenuItemHeight + 16;
    ...
    var menuTop = buttonRect.bottom;
    var menuBottom = menuTop + menuHeight;
    var isBuildToBottom = true;
...
class AdvDropdownButton<T> extends StatefulWidget {
  ...
  /// must be equal to one of the [AdvDropdownMenuItem] values. If [items] or
  ...
  ///
  /// If the [onChanged] callback is null or the list of [AdvDropdownButton.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [AdvDropdownButton.disabledHint] widget if it is non-null.
  /// If [AdvDropdownButton.disabledHint] is also null but [AdvDropdownButton.hint] is
  /// non-null, [AdvDropdownButton.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  final AdvDropdownButtonBuilder? selectedItemBuilder;
...
class _AdvDropdownButtonState<T> extends State<AdvDropdownButton<T>> with WidgetsBindingObserver {
  int? _selectedIndex;
  _DropdownRoute<T>? _dropdownRoute;
  Orientation? _lastOrientation;
  FocusNode? _internalNode;
  Completer? _keyboardCompleter;
  ...
  void didChangeMetrics() {
    if (_keyboardCompleter != null) {
      final value = WidgetsBinding.instance.window.viewInsets.bottom;

      if (value == 0 && !_keyboardCompleter!.isCompleted) _keyboardCompleter!.complete();
    }
  }
  ...
  void initState() {
    ...
    _actionMap = <Type, Action<Intent>>{
      m.ActivateIntent: CallbackAction<ActivateIntent>(
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.items!.where((AdvDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
  ...
  Future<void> _handleTap() async {
    try {
      ...
      if (value > 0) {
        ...
        _keyboardCompleter = Completer();

        await _keyboardCompleter!.future;

        _keyboardCompleter = null;
      }
      //
  ...
  double get _denseButtonHeight {
    final fontSize = _textStyle!.fontSize ?? Theme.of(context).textTheme.bodyMedium!.fontSize!;
...
/// A convenience widget that makes a [AdvDropdownButton] into a [FormField].
class AdvDropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [AdvDropdownButton] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  AdvDropdownButtonFormField({
    Key? key,
    required List<AdvDropdownMenuItem<T>>? items,
    AdvDropdownButtonBuilder? selectedItemBuilder,
    T? value,
    Widget? hint,
    Widget? disabledHint,
    this.onChanged,
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  AdvDropdownButtonFormField<T> get widget => super.widget as AdvDropdownButtonFormField<T>;
  ...
  @override
  void didUpdateWidget(AdvDropdownButtonFormField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
